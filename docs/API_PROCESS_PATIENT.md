...
- `old_account`: resultado da checagem na conta Codental antiga (ver seção "Comportamento" abaixo). É `null` apenas se a checagem nem chegou a rodar por erro fatal antes dela.
...
| `CODENTAL_OLD_ESTABLISHMENT_ID` | condicional | ID da clínica a selecionar após o login na conta antiga, se a conta tiver mais de uma clínica vinculada (Codental exige seleção explícita via `POST /establishments/:id/select`, que `authenticate()` já trata). Para descobrir o valor: faça login manual na conta antiga pelo navegador, abra o DevTools → Network, clique na clínica certa e veja a URL `/establishments/<ID>/select` na requisição, ou confira o cookie `selected_establishment` depois de selecionar. |

> Observação: a forma exata da URL de download de um upload existente
> (`downloadUploadBinary` em `lib/codental.js`) não é documentada pelo
> Codental; se algum arquivo falhar ao baixar, o erro aparece em
> `old_account.errors` com o JSON bruto do item, útil para ajustar o nome do
> campo de URL.
