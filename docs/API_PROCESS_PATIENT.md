...
```json
{
  ...
  "duplicate_records": {
    ...
    "reason": null,
    "duplicates_found": [
      { "id": "4341134", "name": "Maria Silva", "score": 0.93 }
    ],
    "files_found": 9,
    "att_uploaded": 3,
    "att_duplicate": 6,
...
- `att_uploaded` / `att_duplicate` / `att_error` no nível raiz já somam Gmail + cadastros duplicados.
...
- `duplicate_records`: resultado da checagem por cadastros duplicados do mesmo
  paciente na mesma clínica (ver seção "Comportamento" abaixo).
  - `reason: "no_duplicates_found"` — nenhum outro cadastro com nome equivalente (score ≥ 0.90) foi encontrado.
  - `duplicates_found`: lista de outros `patient_id` encontrados com o mesmo nome, de onde os arquivos extras vieram.
...
7. Em seguida, procura na MESMA clínica (mesma sessão principal, sem login
   extra) outros cadastros de paciente com nome equivalente (score ≥ 0.90) —
   comum quando a mesma pessoa foi cadastrada mais de uma vez (ex.: variação
   de sobrenome) — e copia para o cadastro **atual** somente os arquivos que
   ainda não existem nele.

> Observação: a checagem de cadastros duplicados usa a sessão principal já
> gerenciada pelo GitHub Actions — não precisa (nem usa) nenhuma env var extra
> de login. A forma exata da URL de download de um upload existente
> (`downloadUploadBinary` em `lib/codental.js`) não é documentada pelo
> Codental; se algum arquivo duplicado falhar ao baixar, o erro aparece em
> `duplicate_records.errors` com o JSON bruto do item, útil para ajustar o
