...
import { nameSearchVariants } from './extractor.js';
...
/**
 * Busca com fallback: tenta base local primeiro, depois API do Codental.
 * Retorna { patients, source: 'local' | 'api' | 'none' }
 */
export async function searchPatientsWithFallback(name) {
    ...
    // 2. Fallback: API do Codental com variantes
    try {
        const variants = nameSearchVariants(name);
        const seenIds = new Set();
        const all = [];
        for (const v of variants) {
            const pts = await searchPatients(v);
            for (const p of pts) {
                const pid = String(p.id || '');
                if (pid && !seenIds.has(pid)) { seenIds.add(pid); all.push(p); }
            }
        }
        if (all.length > 0) {
            // Normaliza campo de nome para 'name' (API usa fullName, cache usa name)
        all.forEach(p => { if (!p.name && p.fullName) p.name = p.fullName; });
        console.log(`  🌐 API Codental "${name}": ${all.length} resultado(s)${all[0] ? ' — ex: ' + all[0].name : ''}`);
            return { patients: all, source: 'api' };
        }
    } catch (err) {
        console.warn(`  ⚠️ API Codental falhou: ${err.message}`);
    }
