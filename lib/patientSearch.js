...
export async function searchPatientsWithFallback(name) {
    ...
    if (cacheStatus.available) {
        ...
        if (local.length > 0 && local[0].score >= 0.82) {
            // Tem match bom na base local — usa direto
            return { patients: local, source: 'local' };
        }
        if (local.length > 0) {
            console.log(`  ⚠️ Melhor score local: ${local[0].score.toFixed(2)} (abaixo de 0.82) — consultando API do Codental também`);
        } else {
            console.log(`  ⚠️ Nenhum candidato local — consultando API do Codental`);
        }
        // Score baixo ou vazio: consulta API e combina resultados
        const apiResults = await searchPatientsViaAPI(name);
        if (apiResults.length > 0 && apiResults[0].score >= 0.82) {
            // API achou algo bom — usa API
            return { patients: apiResults, source: 'api' };
        }
        // Nenhum achou bom — retorna o melhor disponível (local ou API)
        const combined = [...local, ...apiResults]
            .filter((p, i, arr) => arr.findIndex(q => String(q.id) === String(p.id)) === i) // dedup
            .sort((a, b) => (b.score || 0) - (a.score || 0));
        if (combined.length > 0) {
            return { patients: combined, source: combined[0].score >= 0.82 ? 'combined' : 'suggestion' };
        }
    }
