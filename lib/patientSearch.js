...
export async function searchPatientsWithFallback(name) {
    ...
    if (cacheStatus.available) {
        ...
        if (local.length > 0 && local[0].score >= 0.90) {
            // Tem match bom na base local — usa direto
            return { patients: local, source: 'local' };
        }
        if (local.length > 0) {
            console.log(`  ⚠️ Melhor score local: ${local[0].score.toFixed(2)} (abaixo de 0.90) — consultando API do Codental também`);
        } else {
        ...
        if (apiResults.length > 0 && apiResults[0].score >= 0.90) {
        ...
        if (combined.length > 0) {
            return { patients: combined, source: combined[0].score >= 0.90 ? 'combined' : 'suggestion' };
        }
    }
