...
// ─── BUSCAR MENSAGENS DE UMA PESSOA ESPECÍFICA (POR NOME) ────────────────────

/**
 * Busca emails com anexo que mencionem o nome de uma pessoa específica.
 * @param {string} name
 * @param {Date|null} sinceDate
 * @param {boolean} includeRead
 * @returns {{id:string, threadId:string}[]}
 */
export async function fetchMessagesByName(name, sinceDate = null, includeRead = true) {
    const gmail = await gmailClient();
    ...
    // Busca em frase exata falha quando o nome cadastrado no Codental tem um
    // sobrenome extra que não aparece nos emails (ex.: nome de solteira).
    // Monta tentativas com AND de termos (não frase), relaxando progressivamente
    // ao remover sobrenomes do final, mas sempre exigindo ao menos 2 termos
    // para não cair numa busca só pelo primeiro nome (risco de paciente errado).
    const PREPOSITIONS = new Set(['da', 'de', 'do', 'das', 'dos', 'e']);
    const tokens = name.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '')
        .replace(/[^a-z0-9\s]/g, ' ').trim().split(/\s+/).filter(t => t.length > 1 && !PREPOSITIONS.has(t));

    const attempts = [{ label: 'frase completa', clause: `"${name}"` }];
    for (let i = tokens.length; i >= 2; i--) {
        attempts.push({ label: `AND (${i} termos)`, clause: tokens.slice(0, i).map(t => `"${t}"`).join(' ') });
    }
    if (tokens.length >= 2) {
        const firstLast = `"${tokens[0]}" "${tokens[tokens.length - 1]}"`;
        if (!attempts.some(a => a.clause === firstLast)) attempts.push({ label: 'primeiro+último', clause: firstLast });
    }

    const seen = new Map();
    for (const attempt of attempts) {
        const query = `${readFilter}has:attachment ${attempt.clause}${dateFilter}`;
        console.log(`📧 Query Gmail (por nome, ${attempt.label}): ${query}`);
        ...
        while (true) {
            ...
            console.log(`📧 Página ${page}: ${msgs.length} email(s) (total acumulado: ${seen.size})`);

            if (!currentToken) break;
            await new Promise(r => setTimeout(r, 200));
        }

        if (seen.size > 0) break; // já achou — não relaxa mais a busca
    }
