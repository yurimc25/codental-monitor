...
export async function fetchMessagesByName(name, sinceDate = null, includeRead = true) {
    ...
    const dateFilter = sinceDate instanceof Date
        ...
        : '';
    const readFilter = includeRead ? '' : 'is:unread ';
    const query = `${readFilter}has:attachment "${name}"${dateFilter}`;

    console.log(`📧 Query Gmail (por nome): ${query}`);
    ...
    while (true) {
        ...
        console.log(`📧 Página ${page}: ${msgs.length} email(s) (total: ${allMessages.length})`);
