...
async function authenticate() {
    ...
    // Usa o CSRF do login diretamente — será atualizado antes de cada upload
    // (evita fetch extra que pode causar timeout no Vercel)
    const appCsrf = csrf;
    console.log('🔑 CSRF do login:', appCsrf.slice(0,20)+'...');
...
export async function uploadFile(patientId, buffer, filename, mimeType) {
    // Sempre faz login fresh para garantir CSRF válido
    // O CSRF do Rails expira com a sessão — não pode ser reutilizado do banco
    invalidateSession();
    const s = await getSession();
    const freshCsrf = s.csrf;
    console.log(`🔑 Login fresh para upload | CSRF: ${freshCsrf.slice(0,20)}...`);
    ...
    if (!directRes.ok) {
        ...
        // 422 geralmente significa CSRF expirado — invalida sessão para renovar
    ...
    // Pega CSRF atualizado do response (Rails renova o token a cada request)
    if (typeof directRes.headers.getSetCookie === 'function') {
        const newCookies = directRes.headers.getSetCookie().map(c => c.split(';')[0]).join('; ');
        if (newCookies) {
            s.cookie = mergeCookies(s.cookie, newCookies);
            const newSession = (newCookies.match(/_domain_session=[^;,]+/) || [])[0];
            if (newSession) {
    ...
    // Passo 4: associar arquivo — URLSearchParams conforme fluxo Rails padrão
    const uploadUrl = `${APP_BASE}/patients/${patientId}/uploads`;
    const cookieKeys = s.cookie.split('; ').map(c => c.split('=')[0]).filter(Boolean);
    console.log(`📤 POST ${uploadUrl} | CSRF: ${freshCsrf.slice(0,20)}... | cookies: ${cookieKeys.join(', ')}`);
