...
async function authenticate() {
    ...
    // Extrai CSRF — tenta múltiplos padrões
    ...
    const htmlSnippet = loginHtml.slice(0, 2000);
    console.log('🔍 Login page status:', loginPageRes.status);
    console.log('🔍 CSRF encontrado:', csrfMatch ? csrfMatch[1].slice(0,20)+'...' : 'NÃO ENCONTRADO');
    console.log('🔍 HTML snippet:', htmlSnippet.includes('csrf') ? 'tem csrf' : 'SEM csrf no HTML');

    if (!csrfMatch) {
        // Tenta extrair qualquer token da página
        const anyToken = loginHtml.match(/authenticity_token[^>]+value="([^"]+)"/i)
            || loginHtml.match(/"csrf_token"\s*:\s*"([^"]+)"/i)
            || loginHtml.match(/data-csrf="([^"]+)"/i);
        if (!anyToken) {
            console.error('🔍 HTML primeiros 500 chars:', loginHtml.slice(0, 500));
            throw new Error('CSRF token não encontrado na página de login do Codental');
        }
        console.log('🔍 CSRF alternativo encontrado');
        var csrf = anyToken[1];
    } else {
