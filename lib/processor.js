...
import {
    searchPatients, uploadFile, authenticateWithCredentials,
    listUploadsWithMeta, downloadUploadBinary,
} from './codental.js';
...
export async function processByPatientName(name, { days = 365, includeRead = true } = {}) {
    ...
    // 5. Complementa com arquivos que só existem na conta Codental antiga
    //    (login avulso, nunca mexe na sessão principal salva no banco)
    summary.old_account = await complementFromOldAccount(cleanName, patientId, patientName, batchUploaded);
    summary.att_uploaded  += summary.old_account.att_uploaded;
    summary.att_duplicate += summary.old_account.att_duplicate;
    summary.att_error     += summary.old_account.att_error;

    console.log('✅ processByPatientName concluído:', summary);
    return summary;
}

// ─── COMPLEMENTAR COM ARQUIVOS DA CONTA CODENTAL ANTIGA ──────────────────────
// Faz um login avulso (não persistido) numa segunda conta Codental, procura o
// mesmo paciente lá, e envia para o prontuário da conta ATUAL somente os
// arquivos que ainda não existem nela. Nunca sobrescreve/derruba a sessão
// principal (gerenciada pelo GitHub Actions) — usa cookies isolados.
async function complementFromOldAccount(patientName, newPatientId, newPatientNameCurrent, batchUploaded) {
    const email = process.env.CODENTAL_OLD_EMAIL;
    const password = process.env.CODENTAL_OLD_PASSWORD;

    const result = {
        checked: false,
        reason: null,
        old_patient_id: null,
        old_patient_name: null,
        files_found: 0,
        att_uploaded: 0,
        att_duplicate: 0,
        att_error: 0,
    ...
    if (!email || !password) {
        result.reason = 'no_old_credentials';
        return result;
    }

    let oldSession;
    try {
        oldSession = await authenticateWithCredentials(email, password, process.env.CODENTAL_OLD_ESTABLISHMENT_ID || null);
    } catch (err) {
        result.reason = 'old_login_failed';
        result.errors.push(err.message);
        console.warn(`⚠️ Login na conta Codental antiga falhou: ${err.message}`);
        return result;
    }

    let oldPatients;
    try {
        oldPatients = await searchPatients(patientName, oldSession, { strict: true });
    } catch (err) {
        result.reason = 'old_search_failed';
        result.errors.push(err.message);
        return result;
    }

    const match = bestMatch([{ name: patientName, confidence: 'high' }], oldPatients, 0.90);
    if (!match) {
        result.checked = true;
        result.reason = 'old_patient_not_found';
        return result;
    }

    const oldPatientId = String(match.patient.id);
    const oldPatientName = match.patient.name || match.patient.fullName || patientName;
    result.checked = true;
    result.old_patient_id = oldPatientId;
    result.old_patient_name = oldPatientName;
    console.log(`👤 Paciente encontrado na conta antiga: ${oldPatientName} (ID ${oldPatientId}, score ${match.score?.toFixed(2)})`);

    let oldUploads;
    try {
        oldUploads = await listUploadsWithMeta(oldPatientId, oldSession);
    } catch (err) {
        result.reason = 'old_list_uploads_failed';
        result.errors.push(err.message);
        return result;
    }
    result.files_found = oldUploads.length;

    for (const item of oldUploads) {
        const filename = item.name || item.filename || item.file_name || '';
        if (!filename) continue;
        const ext = filename.toLowerCase().split('.').pop();
        const size = item.byte_size || null;
        const nameKey = `${newPatientId}:${filename.toLowerCase()}`;
        const sizeKey = size ? `${newPatientId}:${size}:${ext}` : null;
        ...
        try {
            const buffer = await downloadUploadBinary(item, oldSession);
            const mimeType = item.content_type || item.mime_type || 'application/octet-stream';
            await uploadFile(newPatientId, buffer, filename, mimeType);
            result.att_uploaded++;
            batchUploaded.set(nameKey, { filename });
            const newSizeKey = `${newPatientId}:${buffer.length}:${ext}`;
            batchUploaded.set(newSizeKey, { filename });
        } catch (err) {
            result.att_error++;
            result.errors.push(`${filename}: ${err.message}`);
            console.warn(`⚠️ Falha ao complementar "${filename}" da conta antiga: ${err.message}`);
        }
    }
