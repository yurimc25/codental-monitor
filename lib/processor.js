...
async function processMessage(messageId, threadId, keywords, batchUploaded) {
    ...
    // Captura CPF do primeiro candidato que trouxer (todos terão o mesmo do email)
    const emailCPF = nameCandidates.find(c => c.cpf)?.cpf || null;
    if (emailCPF) log.cpf_extracted = emailCPF;
    console.log(`  👤 Candidatos: ${nameCandidates.map(c => `${c.name}[${c.confidence}]`).join(', ') || 'nenhum'}${emailCPF ? ` | CPF: ${emailCPF}` : ''}`);
