...
import {
    ...
    searchPatients, authenticateWithCredentials,
} from './codental.js';
...
export async function processByPatientName(name, { days = 365, includeRead = true } = {}) {
    ...
    // 5. Complementa com arquivos que só existem numa conta/clínica ANTIGA
...
// ─── COMPLEMENTAR COM ARQUIVOS DE UMA CONTA CODENTAL ANTIGA ──────────────────
// Faz um login avulso (não persistido, não afeta a sessão principal gerenciada
// pelo GitHub Actions) numa segunda conta Codental — que pode ter mais de uma
// clínica vinculada, exigindo seleção explícita via POST
// /establishments/:id/select (authenticate() já trata isso) — procura o
// mesmo paciente lá e copia pro cadastro atual só os arquivos que faltam.
async function complementFromOldAccount(patientName, primaryPatientId, primaryPatientName, batchUploaded) {
    const email = process.env.CODENTAL_OLD_EMAIL;
    const password = process.env.CODENTAL_OLD_PASSWORD;
    const establishmentId = process.env.CODENTAL_OLD_ESTABLISHMENT_ID || null;
    ...
    let oldSession;
    try {
        oldSession = await authenticateWithCredentials(email, password, establishmentId);
    } catch (err) {
