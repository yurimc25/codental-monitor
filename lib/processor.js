...
export async function run({ sinceDate = null, days = 2, includeRead = true, offset = 0, batchSize = 80 } = {}) {
    ...
    summary.oldest_date_seen  = null;
    summary.newest_date_seen  = null;

    // helper: atualiza datas min/max
    const trackDate = (d) => {
        if (!d) return;
    ...
    summary.oldest_date_seen = null;   // data do email mais antigo processado neste lote
    summary.newest_date_seen = null;   // data do email mais recente
...
async function processMessage(messageId, threadId, keywords, batchUploaded) {
    ...
    if (!patientMatch) {
        ...
        if (log.patient_name_extracted) {
            ...
            if (alreadyConfirmed) {
                // Paciente já foi confirmado antes — marca como duplicate e encerra
                log.status = 'duplicate_all';
                ...
                await saveLog(log);
                console.log(`  ⏭ Paciente já confirmado antes: ${log.patient_name_extracted}`);
                return { ...result, status: 'duplicate_all' };
            }
        }
