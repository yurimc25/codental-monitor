...
export async function run({ sinceDate = null, days = 2, includeRead = true } = {}) {
    ...
    // Mapa de arquivos já enviados neste lote: detecta duplicatas entre emails
    const batchUploaded = new Map(); // "patientId:size:ext" ou "patientId:filename" → { filename, messageId }
...
async function processMessage(messageId, threadId, keywords) {
    ...
    for (const att of attachments) {
        ...
        try {
            ...
            if (batchPrev) {
                console.log(`  ⏭ Duplicata no lote: ${att.filename} == ${batchPrev.filename}`);
                attLog.status = 'skipped_duplicate';
                attLog.skipped_reason = `Duplicata de ${batchPrev.filename} (mesmo email diferente)`;
                result.att_duplicate++;
                log.attachments.push(attLog);
                continue;
            }

            // 2. Verifica duplicata no Codental (uploads já existentes no prontuário)
            if (await isDuplicate(patientId, att.filename, buffer)) {
                console.log(`  ⏭ Duplicata no Codental: ${att.filename}`);
                attLog.status = 'skipped_duplicate';
                attLog.skipped_reason = 'Arquivo com mesmo nome/tamanho já existe no prontuário';
                result.att_duplicate++;
                log.attachments.push(attLog);
                continue;
            }
