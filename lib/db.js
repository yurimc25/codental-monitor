...
export async function saveLog(doc) {
    const col = (await db()).collection('email_logs');
    const { gmail_message_id, ...rest } = doc;
    // upsert: atualiza se já existe, insere se não existe
    return col.updateOne(
        { gmail_message_id },
        { $set: { ...rest, processed_at: new Date() } },
        { upsert: true }
    );
}

export async function getLogs({ page = 1, limit = 25, status, search, sort = 'newest' } = {}) {
    const col = (await db()).collection('email_logs');
    const filter = {};
    ...
    const [logs, total] = await Promise.all([
        col.find(filter).sort({ processed_at: sortDir }).skip((page - 1) * limit).limit(limit).toArray(),
...
export async function getStats() {
    ...
    // Busca info da base de pacientes e checkpoint
    const settingsCol = (await db()).collection('settings');
    const [patientsSync, checkpoint] = await Promise.all([
        settingsCol.findOne({ _id: 'patients_sync' }),
        settingsCol.findOne({ _id: 'processing_checkpoint' }),
    ]);
    const patientsCache = (await db()).collection('patients_cache');
    const patientsTotal = await patientsCache.estimatedDocumentCount();
    ...
    return {
        ...
        synced_at: patientsSync?.synced_at || null,
        patients_total: patientsTotal || patientsSync?.total || 0,
        checkpoint_oldest: checkpoint?.oldest_date_seen || null,
        checkpoint_offset: checkpoint?.next_offset || null,
        checkpoint_total: checkpoint?.total_found || null,
    };
    } catch(err) {
        console.error('getStats error:', err);
        return { total_emails: 0, uploaded: 0, partial: 0, failed: 0, no_patient: 0,
            pending_review: 0, duplicate_all: 0, att_uploaded: 0, att_duplicate: 0,
            att_error: 0, last_run: null };
    }
}
...
export async function getExistingLog(gmailMessageId) {
    const col = (await db()).collection('email_logs');
    return col.findOne({ gmail_message_id: gmailMessageId });
}
...
export async function getCheckpoint() {
    const col = (await db()).collection('settings');
    return col.findOne({ _id: 'processing_checkpoint' });
}

export async function clearCheckpoint() {
    const col = (await db()).collection('settings');
    await col.deleteOne({ _id: 'processing_checkpoint' });
}