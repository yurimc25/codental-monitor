...
// Bloco de nome: Capitalizado OU TUDO MAIÚSCULO
const NAME_BLOCK = String.raw`[A-ZÁÉÍÓÚÀÂÊÔÃÕÜÇ][A-ZÁÉÍÓÚÀÂÊÔÃÕÜÇa-záéíóúàâêôãõüç]+(?:\s+(?:de|da|do|dos|das|e|[A-ZÁÉÍÓÚÀÂÊÔÃÕÜÇ][A-ZÁÉÍÓÚÀÂÊÔÃÕÜÇa-záéíóúàâêôãõüç]+))*)`;

const PATTERNS = [
    // "Paciente: João Silva" / "Paciente: JOÃO SILVA"
    new RegExp(String.raw`paciente[:\s\-–]+(` + NAME_BLOCK + `)`, 'gi'),

    // "Radiografias do paciente: NOME"
    new RegExp(String.raw`(?:radiografias?|exames?|laudo|cbct|tomografia|voxels?|fenelon|radiomaster|documentaç[aã]o)\s+do\s+paciente[:\s\-–]+(` + NAME_BLOCK + `)`, 'gi'),

    // "Keyword - Nome" / "Keyword | Nome"
    new RegExp(String.raw`(?:tomografia|cbct|voxels?|fenelon|radiomaster|documentaç[aã]o|laudo|exame|radiografia)[:\s\-–|\/]+(` + NAME_BLOCK + `)`, 'gi'),

    // "Nome - Keyword" (nome antes do separador)
    new RegExp(String.raw`(` + NAME_BLOCK + String.raw`)\s*[\-–|]\s*(?:tomografia|cbct|voxels?|fenelon|radiomaster|documentaç[aã]o|laudo|exame|radiografia)`, 'gi'),

    // "nome: NOME"
    new RegExp(String.raw`nome[:\s\-–]+(` + NAME_BLOCK + `)`, 'gi'),
];
...
function extractCapitalized(text) {
    ...
    for (const tok of tokens) {
        // Aceita Capitalizado OU TUDO MAIÚSCULO (mínimo 2 chars)
        const isCapitalized = /^[A-ZÁÉÍÓÚÀÂÊÔÃÕÜÇ][a-záéíóúàâêôãõüç]{1,}$/.test(tok);
        const isAllCaps = /^[A-ZÁÉÍÓÚÀÂÊÔÃÕÜÇ]{2,}$/.test(tok);
