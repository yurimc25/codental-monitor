...
// AUTO: score >= 0.90 → aceita automaticamente
// SUGESTÃO: score entre 0.45 e 0.90 → guarda para revisão manual
export function bestMatch(candidates, patients, threshold = 0.90) {
...
export function matchWithSuggestions(candidates, patients) {
    ...
    for (const cand of candidates) {
        for (const p of patients) {
            ...
            if (!pName) continue;
            const score = Math.min(1, scorePair(cand.name, pName) + (cand.confidence === 'high' ? 0.03 : 0));
            if (score >= 0.90 && score > bestAutoScore) { bestAutoScore = score; bestAuto = { patient: p, score, candidateName: cand.name }; }
