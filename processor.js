...
import { extractNames, bestMatch } from './extractor.js';
...
export async function run() {
    ...
    const summary = {
        emails_found: 0,
        emails_skipped: 0,     // já processados anteriormente
        emails_processed: 0,
        att_uploaded: 0,
        att_duplicate: 0,
        att_error: 0,
    ...
    // 1. Busca não lidos
    const messages = await fetchUnreadMessages(keywords);
...
async function processMessage(messageId, threadId, keywords) {
    ...
    for (const cand of nameCandidates) {
        ...
        patientMatch = bestMatch([cand], patients);
        if (patientMatch) {
            log.patient_name_extracted = cand.name;
            log.patient_id_codental = String(patientMatch.patient.id);
            log.patient_name_codental = patientMatch.patient.name || patientMatch.patient.full_name || null;
            console.log(`  ✅ Paciente: ${log.patient_name_codental} (ID ${log.patient_id_codental})`);
            break;
        }
    }
    ...
    if (!patientMatch) {
        ...
        await saveLog(log);
        return { ...result, status: 'no_patient' };
    }
    ...
    for (const att of attachments) {
        ...
        try {
            // Verifica duplicata
            if (await isDuplicate(patientId, att.filename)) {
            ...
            // Download
            console.log(`  ⬇ Baixando: ${att.filename} (${(att.size / 1024).toFixed(1)} KB)`);
